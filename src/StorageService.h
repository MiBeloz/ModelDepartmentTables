...
public:
    StorageService()
        ...
        , m_amountStorage(std::make_unique<CustomStorage<int>>()) { }
    ...
    CustomStorage<int>& amounts() {
        return *m_amountStorage;
    }
    const CustomStorage<int>& amounts() const {
        return *m_amountStorage;
    }
...
private:
    ...
    std::unique_ptr<CustomStorage<int>> m_amountStorage;
};
