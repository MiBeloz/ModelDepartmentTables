...
public:
  ...
  bool containsIdByData(const Data &data) const {
    return m_uniqueKeys ? m_uniqueCache.values().contains(data)
                        : m_multiCache.values().contains(data);
  }
  void clear() override {
    if (m_uniqueKeys) {
      m_uniqueCache.clear();
  ...
  size_t size() const override { return m_uniqueKeys ? m_uniqueCache.size() : m_multiCache.size(); }
...
public:
  ...
  QString name() const override { return Storage::name(); }
...
public:
  ...
  static bool loadFromFile(ThreadSafeCache<Data> &cache, const QString &filename) { return false; }
};
