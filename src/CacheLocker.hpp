...
public:
    ...
    ThreadSafeCache(const QString &name, bool uniqueKeys = true) : Storage(name, uniqueKeys) { }
    ...
    bool writeToFile(const QString &filename) const {
        ...
        if (file.open(QIODevice::WriteOnly)) {
            ...
            if (Storage::m_uniqueKeys) {
                for (auto it = Storage::m_uniqueCache.begin(); it != Storage::m_uniqueCache.end();
                     ++it) {
                    // out << it.key() << it.value();
                }
            } else {
                for (auto it = Storage::m_multiCache.begin(); it != Storage::m_multiCache.end();
                     ++it) {
                    // out << it.key() << it.value();
                }
            }
