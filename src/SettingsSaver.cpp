...
SettingsSaver::SettingsSaver(const QString& filename, QObject* parent) : QObject(parent) {
...
void SettingsSaver::readGroup(SettingsGroup& settingsGroup) const {
    ...
    for (auto& parameter : settingsGroup.parameters()) {
        if (m_settings->contains(parameter.m_name)) {
            settingsGroup.writeParameter(parameter.m_name,
                                         m_settings->value(parameter.m_name,
                                                           parameter.m_defaultValue));
        } else {
